package com.jinxin.cloudmusic.util.lrcview;
...
public class LrcView extends View {
    ...
    public void updateTime(long time) {
        ...
        if (time < mNextTime) {
            //return;
            //上拉
            /*for (int i =mLrcEntryList.size()-1 ; i >=mCurrentLine ; i--) {
        ...
        for (int i = mCurrentLine; i < mLrcEntryList.size(); i++) {
            if (mLrcEntryList.get(i).getTime() > time) {
                ...
                if (i!=JxscApp.lyricSum){
                    newlineOnUI(i);
                    JxscApp.lyricSum=i;
                    Log.e("---开始newlineOnUI",i+"行数："+mCurrentLine);
                }
                break;
            }else if(i == mLrcEntryList.size() - 1){
                // 最后一行
                ...
                Log.e("---结束newlineOnUI", "newlineOnUI");
                break;
            }
        }
    ...
    private void newlineAnimation(int index) {
        stopAnimation();
        if (index<mLrcEntryList.size()) {//判空处理
            ...
            mAnimator.start();
            //Log.e("----Animation", "动画" + mAnimator.toString());
        }
    }
